...
if [[ ${PV} == *9999 ]]; then
	...
	KEYWORDS="~alpha ~arm ~hppa ~ia64 ~ppc ~ppc64 ~x86-linux"
else
	SRC_URI="http://www.cups.org/software/${MY_PV}/${MY_P}-source.tar.bz2"
	KEYWORDS="~alpha ~amd64 ~arm ~hppa ~ia64 ~mips ~ppc ~ppc64 ~s390 ~sh ~sparc ~x86 ~amd64-fbsd ~x86-fbsd ~m68k-mint ~x86-linux"
fi
...
multilib_src_install_all() {
	...
	keepdir ${EPREFIX}/usr/libexec/cups/driver ${EPREFIX}/usr/share/cups/{model,profiles} \
		${EPREFIX}/var/log/cups /var/spool/cups/tmp

	keepdir ${EPREFIX}/etc/cups/{interfaces,ppd,ssl}
